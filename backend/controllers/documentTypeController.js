...
// In your controller file (e.g., documentTypeController.js)
 
// DELETE Mail Management
exports.deleteMailManagement = async (req, res) => {
    const { docId } = req.params;
    try {
      const deletedMailManagement = await MailManagement.findByIdAndDelete(docId);
      if (!deletedMailManagement) {
        return res.status(404).json({ message: 'Mail Management not found' });
      }
      res.status(200).json({
        message: 'Mail Management document deleted successfully',
        document: deletedMailManagement,
      });
    } catch (error) {
      res.status(500).json({
        error: 'Error deleting Mail Management document',
        details: error.message,
      });
    }
  };
 
  // DELETE Virtual Reception
  exports.deleteVirtualReception = async (req, res) => {
    const { docId } = req.params;
    try {
      const deletedVirtualReception = await VirtualReception.findByIdAndDelete(docId);
      if (!deletedVirtualReception) {
        return res.status(404).json({ message: 'Virtual Reception not found' });
      }
      res.status(200).json({
        message: 'Virtual Reception document deleted successfully',
        document: deletedVirtualReception,
      });
    } catch (error) {
      res.status(500).json({
        error: 'Error deleting Virtual Reception document',
        details: error.message,
      });
    }
  };
...
// In the same file or a separate controller file, e.g., documentTypeController.js
 
exports.deletePersonalBank = async (req, res) => {
    ...
    try {
      ...
      const deletedDoc = await PersonalBank.findByIdAndDelete(docId);
